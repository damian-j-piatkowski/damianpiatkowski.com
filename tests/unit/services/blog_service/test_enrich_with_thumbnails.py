"""Unit tests for the enrich_with_thumbnails service function.

This module verifies that enrich_with_thumbnails correctly attaches a resolved
thumbnail base path to each blog post, using either a remote or local static path.

Tests included:
    - test_enrich_with_thumbnails_empty_list: Ensures an empty input returns an empty list.
    - test_enrich_with_thumbnails_local_fallback: Verifies fallback to the default thumbnail if a specific
        directory is missing.
    - test_enrich_with_thumbnails_local_found: Verifies correct path resolution when the local thumbnail exists.
    - test_enrich_with_thumbnails_mixed_hybrid: Verifies behavior when some posts resolve to remote and
        others fallback to local.
    - test_enrich_with_thumbnails_mixed_local: Verifies behavior when some slugs have thumbnail files
        and others fallback.
    - test_enrich_with_thumbnails_missing_slug: Ensures posts without a slug are skipped safely.
    - test_enrich_with_thumbnails_remote_base: Verifies URL resolution when using a remote image base path.
...
from app.services.blog_service import enrich_with_thumbnails


def test_enrich_with_thumbnails_empty_list(app):
    """Verifies that an empty input list returns an empty list."""
    result = enrich_with_thumbnails([])
    assert result == []


@patch("app.services.blog_service.os.path.exists")
@patch("app.services.blog_service.url_for")
def test_enrich_with_thumbnails_local_fallback(mock_url_for, mock_exists, app):
    """Verifies fallback to the default thumbnail when desktop.jpg is missing."""
    mock_exists.return_value = False
    mock_url_for.side_effect = lambda *args, **kwargs: f"/static/{kwargs['filename']}"

    posts = [{"slug": "missing-thumb"}]
    enriched = enrich_with_thumbnails(posts)
...
@patch("app.services.blog_service.os.path.exists")
@patch("app.services.blog_service.url_for")
def test_enrich_with_thumbnails_local_found(mock_url_for, mock_exists, app):
    """Verifies the correct thumbnail path is used when desktop.jpg exists locally."""
...
@patch("app.services.blog_service.os.path.exists")
@patch("app.services.blog_service.url_for")
def test_enrich_with_thumbnails_mixed_local(mock_url_for, mock_exists, app):
...
@patch("app.services.blog_service.os.path.exists")
@patch("app.services.blog_service.url_for")
def test_enrich_with_thumbnails_mixed_hybrid(mock_url_for, mock_exists, app):
    ...
    enriched_remote = enrich_with_thumbnails(remote)
    ...
    enriched_local = enrich_with_thumbnails(local)
...
def test_enrich_with_thumbnails_missing_slug(app):
    """Verifies that posts without a slug are skipped and not mutated."""
    posts = [{"title": "Untitled"}]
    result = enrich_with_thumbnails(posts)
    assert "thumb_base" not in result[0]


def test_enrich_with_thumbnails_remote_base(app):
